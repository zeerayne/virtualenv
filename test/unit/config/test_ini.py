...
def test_ini_file(tmp_path, monkeypatch, conf, capsys, caplog):
    ...
    assert result.seed_packages == ["magic"]
    assert result.search_dirs == [str(conf.parent), str(conf)]
...
def test_bad_ini_conversion(tmp_path, monkeypatch, capsys, caplog):
    ...
    assert [r.message for r in caplog.records] == [
        "file failed to convert 'critical' as {!r} because {!r}".format(int, _exc("critical")),
        "file failed to convert 'info' as {!r} because {!r}".format(int, _exc("info")),
        "file failed to convert 'whatever' as {!r} because {!r}".format(
            _convert_to_boolean, ValueError("Not a boolean: whatever")
        ),
    ], caplog.text
