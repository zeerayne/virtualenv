...
from virtualenv.boostrap.link_app_data import bootstrap as bootstrap_via_link_app_data
...
class Creator(object):
    ...
    def get_host_create_cmd(self):
        cmd = [str(self.interpreter.system_executable), "-m", "venv"]
        if self.options.system_site:
            cmd.append("--system-site-packages")
        if "pip" not in self.options.seed_packages:
